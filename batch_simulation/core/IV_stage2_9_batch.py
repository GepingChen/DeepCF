...
# Import from Stage 1
from DGP_batch import DGPConfig, set_seed, sigmaY_of_X, LATENT_H_WEIGHT
...
def load_dgp_test_data(first_stage: str, second_stage: str, base_dir: str | os.PathLike[str] = DEFAULT_DATA_DIR) -> Tuple[Path, Dict[str, Optional[np.ndarray]]]:
    """
...
def prepare_stage2_components(cfg: Stage2_9Config):
    ...
    train_prefix = f"iv_stage1_train_{codes}{sample_tag}{seed_tag}"
    train_csv_path = _latest_matching_file(cfg.input_dir, train_prefix)
    if train_csv_path is None and cfg.train_seed is not None:
...
def simulate_y_given_x_eps(cfg: DGPConfig,
    ...
    if cfg.second_stage == "B1":
        ...
        return np.sin(x_arr) * (1.0 + cfg.b6_h_weight * h_arr) + cfg.b6_eps_slope * x_arr * eps_arr
...
def monte_carlo_y_given_x(cfg: DGPConfig,
    ...
    """
    needs_h = cfg.second_stage in {"B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8"}
...
def save_stage2_9_results(
    ...
    *,
    ...
    sample_tag = f"_n{int(n_train_samples_meta)}" if n_train_samples_meta is not None else ""
    seed_tag = f"_seed{train_seed}" if train_seed is not None else ""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S") if use_timestamp else None

    base_prefix = f"s2_{codes}{sample_tag}{seed_tag}"
